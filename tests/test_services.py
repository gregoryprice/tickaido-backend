...
def test_integration_service_config_validation():
    ...
    salesforce_fields = service._get_required_config_fields(IntegrationType.SALESFORCE)
    assert "client_id" in salesforce_fields
    assert "client_secret" in salesforce_fields
    assert "instance_url" in salesforce_fields
    
    jira_fields = service._get_required_config_fields(IntegrationType.JIRA)
    ...
    service._validate_integration_config(IntegrationType.SALESFORCE, valid_config)
    
    # Invalid config should raise exception
    invalid_config = {"client_id": "test"}  # Missing required fields
    
    try:
        service._validate_integration_config(IntegrationType.SALESFORCE, invalid_config)
